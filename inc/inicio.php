...
print "</div>";
	...
	for ($i = 0; $i <= $num_filas/5; $i++)
	{
		...
		if ($page == $temp)
		{
			print "<li class=\"active\"><a href=\"$url/page/$temp\">$temp<span class=\"sr-only\">(current)</span></a></li>";
		}else
		{
			print "<li><a href=\"$url/page/$temp\">$temp<span class=\"sr-only\">(current)</span></a></li>";
		}
	}
	print "</ul></center>";
	print "</div>";
...
function imprimir_url($path)
{
	...
	$query = "SELECT * FROM post WHERE url='$path'";
		$resultado = $mysqli->query($query);
print "<div class=\"container\">";
	...
	print "<center>";
	print "<ul class=\"pagination\">";
	print "<li><a href=\"$url\">Volver<span class=\"sr-only\">(current)</span></a>";
	print "</ul></center>";
	print "</div>";
	print "</div>";
}
